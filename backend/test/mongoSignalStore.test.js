...
test('creates identity, single-open, and history indexes', async () => {
  ...
  const db = {
    collection(name) {
      ...
      return { async createIndex(keys, options) { captured.push({ keys, options }); } };
    }
  };
    ...
    collection() {
      return {
        async findOne(filter) {
          assert.deepEqual(filter, { symbol: 'XAUUSD', isOpen: true });
          return { _id: 'mongo-id', signalId: 'SIG-1', symbol: 'XAUUSD', isOpen: true };
        }
      };
    }
  };
  assert.deepEqual(await loadActive(db), { signalId: 'SIG-1', symbol: 'XAUUSD', isOpen: true });
});
...
test('loads newest signal history first and applies the bounded limit', async () => {
  ...
  const history = await loadHistory(db, 'XAUUSD', 1);
